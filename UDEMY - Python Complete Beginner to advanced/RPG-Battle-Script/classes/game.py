...
class Person:
    ...
    # Choose Attack, Magic, or Use Item, and chose spell or item
    ...
    def get_enemy_stats(self):
        # Make bar reflect current stats
        hp_bar = ""
        ...
        # White Space in stats to keep formating
    ...
    def get_stats(self):
        ...
        # Show current stats of players
        print("                     _________________________            __________")
        print(bcolors.BOLD + self.name + "   " + current_hp + "  |" + bcolors.OKGREEN +
              hp_bar + bcolors.ENDC + bcolors.BOLD + "|   " + current_mp + "|" + bcolors.OKBLUE +
              mp_bar + bcolors.ENDC + bcolors.BOLD + "|" + bcolors.ENDC)

    def choose_enemy_spell(self):
        # Generate random number from enemy's length of magic spells to chose spell
        magic_choice = random.randrange(0, len(self.magic) - 1)
        spell = self.magic[magic_choice]
        magic_dmg = spell.generate_damage()

        # Determine enemy HP %, only use heal if under 50%
